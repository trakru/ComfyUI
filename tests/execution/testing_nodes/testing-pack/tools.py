...
def VariantSupport():
    def decorator(cls):
                ...
                for category in ["required", "optional"]:
                    ...
                    for key, value in types[category].items():
                        if isinstance(value, tuple):
                            types[category][key] = (MakeSmartType(value[0]),) + value[
                                1:
                            ]
                return types
        ...
        if hasattr(cls, "RETURN_TYPES"):
            ...
            setattr(
                cls, "RETURN_TYPES", tuple(MakeSmartType(x) for x in old_return_types)
            )
        if hasattr(cls, "VALIDATE_INPUTS"):
            ...
            raise NotImplementedError(
                "VariantSupport does not support VALIDATE_INPUTS yet"
            )
        else:
            ...
            def validate_inputs(input_types):
                ...
                for key, value in input_types.items():
                    ...
                    if (
                        ...
                        and MakeSmartType(value) != expected_type
                    ):
                        return (
                            f"Invalid type of {key}: {value} (expected {expected_type})"
                        )
                return True
