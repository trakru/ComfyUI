...
class TorchDictFolderFilename(ResourceKey):
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, TorchDictFolderFilename):
            return False
        return (
            self.folder_name == other.folder_name and self.file_name == other.file_name
        )
...
class ResourcesLocal(Resources):
    ...
    def get(self, key: ResourceKey, default: Any = ...) -> Any:
        ...
        if isinstance(key, TorchDictFolderFilename):
            if default is ...:
                to_return = comfy.utils.load_torch_file(
                    folder_paths.get_full_path_or_raise(key.folder_name, key.file_name),
                    safe_load=True,
                )
            else:
