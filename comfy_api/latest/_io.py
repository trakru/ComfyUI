...
class MatchType(ComfyTypeIO):
    class Template:
        def __init__(
            self, template_id: str, allowed_types: _ComfyType | list[_ComfyType]
        ):
            self.template_id = template_id
            self.allowed_types = (
                [allowed_types]
                if isinstance(allowed_types, _ComfyType)
                else allowed_types
            )
...
class _ComfyNodeBaseInternal(_ComfyNodeInternal):
    ...
    @classmethod
    def INPUT_TYPES(
        cls, include_hidden=True, return_schema=False
    ) -> dict[str, dict] | tuple[dict[str, dict], Schema]:
