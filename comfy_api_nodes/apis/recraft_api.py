...
class RecraftControls:
    ...
    def create_api_model(self):
        if (
            ...
            and self.artistic_level is None
            and self.no_text is None
        ):
            return None
        colors_api = None
        ...
        if self.background_color:
            ...
            background_color_api = (
                first_background.create_api_model() if first_background else None
            )

        return RecraftControlsObject(
            colors=colors_api,
            background_color=background_color_api,
            artistic_level=self.artistic_level,
...
class RecraftColorObject(BaseModel):
    rgb: list[int] = Field(
        ...,
        description="An array of 3 integer values in range of 0...255 defining RGB Color Model",
    )


class RecraftControlsObject(BaseModel):
    colors: Optional[list[RecraftColorObject]] = Field(
        None, description="An array of preferable colors"
    )
    background_color: Optional[RecraftColorObject] = Field(
    ...
    artistic_level: Optional[conint(ge=0, le=5)] = Field(
        None,
        description="Defines artistic tone of your image. At a simple level, the person looks straight at the camera in a static and clean style. Dynamic and eccentric levels introduce movement and creativity. The value should be in range [0..5].",
    )
...
class RecraftImageGenerationRequest(BaseModel):
    ...
    size: Optional[RecraftImageSize] = Field(
        None, description='The size of the generated image (e.g., "1024x1024")'
    )
    n: conint(ge=1, le=6) = Field(..., description="The number of images to generate")
    negative_prompt: Optional[str] = Field(
        None, description="A text description of undesired elements on an image"
    )
    model: Optional[RecraftModel] = Field(
        RecraftModel.recraftv3,
    ...
    substyle: Optional[str] = Field(
        None,
        description="The substyle to apply to the generated image, depending on the style input",
    )
    controls: Optional[RecraftControlsObject] = Field(
        None, description="A set of custom parameters to tweak generation process"
    )
    style_id: Optional[str] = Field(
        None, description="Use a previously uploaded style as a reference; UUID"
    )
...
class RecraftImageGenerationResponse(BaseModel):
    ...
    data: Optional[list[RecraftReturnedObject]] = Field(
        None, description="Array of generated image information"
    )
    image: Optional[RecraftReturnedObject] = Field(
        None, description="Single generated image"
    )
