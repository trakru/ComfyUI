    ...
    if input.ndim == 3 or input.ndim == 2:
        ...
        if scale_input is None:
            ...
            quantized_input = QuantizedTensor(
                input.to(dtype).contiguous(),
                "TensorCoreFP8Layout",
                layout_params_weight,
            )
        else:
    ...
    class scaled_fp8_op(manual_cast):
        class Linear(manual_cast.Linear):
            ...
            def convert_weight(self, weight, inplace=False, **kwargs):
                if inplace:
                    weight *= self.scale_weight.to(
                        device=weight.device, dtype=weight.dtype
                    )
                    return weight
                else:
                    return weight * self.scale_weight.to(
                        device=weight.device, dtype=weight.dtype
                    )
            ...
            def set_weight(
                ...
                **kwargs,
                ...
                weight = comfy.float.stochastic_rounding(
                    weight
                    / self.scale_weight.to(device=weight.device, dtype=weight.dtype),
                    self.weight.dtype,
                    seed=seed,
                )
                if return_weight:
    ...
    class Linear(torch.nn.Module, CastWeightBiasOp):
        def __init__(
            ...
            if bias:
                self.bias = torch.nn.Parameter(
                    torch.empty(out_features, **self.factory_kwargs)
                )
            else:
        ...
        def _load_from_state_dict(
            ...
            if layer_name not in MixedPrecisionOps._layer_quant_config:
                ...
                quant_format = MixedPrecisionOps._layer_quant_config[layer_name].get(
                    "format", None
                )
                ...
                self.weight = torch.nn.Parameter(
                    QuantizedTensor(
                        weight.to(device=device), self.layout_type, layout_params
                    ),
                    requires_grad=False,
                )
...
def pick_operations(
    ...
    if (
        model_config
        and hasattr(model_config, "layer_quant_config")
        and model_config.layer_quant_config
    ):
        MixedPrecisionOps._layer_quant_config = model_config.layer_quant_config
        MixedPrecisionOps._compute_dtype = compute_dtype
        logging.info(
            f"Using mixed precision operations: {len(model_config.layer_quant_config)} quantized layers"
        )
    ...
    if scaled_fp8 is not None:
        return scaled_fp8_ops(
            fp8_matrix_mult=fp8_compute and fp8_optimizations,
            scale_input=fp8_optimizations,
            override_dtype=scaled_fp8,
        )
