...
class CLIPTextModel_(torch.nn.Module):
    ...
    def forward(
        ...
        if num_tokens is not None:
            pooled_output = x[
                list(range(x.shape[0])), list(map(lambda a: a - 1, num_tokens))
            ]
        else:
            pooled_output = x[
...
class CLIPVisionEmbeddings(torch.nn.Module):
    ...
    def forward(self, pixel_values):
        ...
        if self.class_embedding is not None:
            embeds = torch.cat(
                [
                    comfy.ops.cast_to_input(self.class_embedding, embeds).expand(
...
class LlavaProjector(torch.nn.Module):
    def __init__(self, in_dim, out_dim, dtype, device, operations):
        ...
        self.linear_1 = operations.Linear(
            in_dim, out_dim, bias=True, device=device, dtype=dtype
        )
        self.linear_2 = operations.Linear(
            out_dim, out_dim, bias=True, device=device, dtype=dtype
        )
...
class CLIPVisionModelProjection(torch.nn.Module):
    def __init__(self, config_dict, dtype, device, operations):
        ...
        if "llava3" == config_dict.get("projector_type", None):
            self.multi_modal_projector = LlavaProjector(
                config_dict["hidden_size"], 4096, dtype, device, operations
            )
        else:
