...
class IndexListContextHandler(ContextHandlerABC):
    ...
    def should_use_context(
        ...
        if x_in.size(self.dim) > self.context_length:
            logging.info(
                f"Using context windows {self.context_length} for {x_in.size(self.dim)} frames."
            )
            return True
        return False
                    ...
                    # look for control
                        ...
                        for cond_key, cond_value in new_cond_item.items():
                            if isinstance(cond_value, torch.Tensor):
                                if cond_value.ndim < self.dim and cond_value.size(
                                    0
                                ) == x_in.size(self.dim):
                            ...
                            # if has cond that is a Tensor, check if needs to be subset
                                ...
                                if (
                                    ...
                                    new_cond_item[cond_key] = cond_value._copy_with(
                                        window.get_tensor(cond_value.cond, device)
                                    )
    ...
    def set_step(self, timestep: torch.Tensor, model_options: dict[str]):
        mask = torch.isclose(
            model_options["transformer_options"]["sample_sigmas"], timestep, rtol=0.0001
        )
    ...
    def get_context_windows(
        ...
        context_windows = [
            IndexListContextWindow(window, dim=self.dim) for window in context_windows
        ]
