...
class ResBlock(nn.Module):
    def __init__(self, c, c_hidden):
        ...
        self.depthwise = nn.Sequential(
            nn.ReplicationPad2d(1), ops.Conv2d(c, c, kernel_size=3, groups=c)
        )
...
class StageA(nn.Module):
    def __init__(
        ...
        self.in_block = nn.Sequential(
            nn.PixelUnshuffle(2), ops.Conv2d(3 * 4, c_levels[0], kernel_size=1)
        )
        down_blocks = []
        ...
        # Decoder blocks
        up_blocks = [nn.Sequential(ops.Conv2d(c_latent, c_levels[-1], kernel_size=1))]
        for i in range(levels):
    ...
    def __init__(self, c_in=3, c_cond=0, c_hidden=512, depth=6):
        ...
        layers = [
            nn.utils.spectral_norm(
                ops.Conv2d(c_in, c_hidden // (2**d), kernel_size=3, stride=2, padding=1)
            ),
            nn.LeakyReLU(0.2),
        ]
        for i in range(depth - 1):
            ...
            layers.append(
                nn.utils.spectral_norm(
                    ops.Conv2d(c_in, c_out, kernel_size=3, stride=2, padding=1)
                )
            )
            layers.append(nn.InstanceNorm2d(c_out))
            layers.append(nn.LeakyReLU(0.2))
        self.encoder = nn.Sequential(*layers)
        self.shuffle = ops.Conv2d(
