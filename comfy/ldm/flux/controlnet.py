...
class MistolineCondDownsamplBlock(nn.Module):
    def __init__(self, dtype=None, device=None, operations=None):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.SiLU(),
            operations.Conv2d(
                16, 16, 3, padding=1, stride=2, dtype=dtype, device=device
            ),
            nn.SiLU(),
            operations.Conv2d(16, 16, 3, padding=1, dtype=dtype, device=device),
            nn.SiLU(),
...
class ControlNetFlux(Flux):
    def __init__(
        ...
        **kwargs,
        ...
        if self.mistoline:
            control_block = lambda: MistolineControlnetBlock(
                self.hidden_size, dtype=dtype, device=device, operations=operations
            )
        else:
            control_block = lambda: operations.Linear(
        ...
        if self.num_union_modes > 0:
            self.controlnet_mode_embedder = operations.Embedding(
                self.num_union_modes, self.hidden_size, dtype=dtype, device=device
            )
        ...
        self.pos_embed_input = operations.Linear(
        ...
        if not self.latent_input:
            if self.mistoline:
                self.input_cond_block = MistolineCondDownsamplBlock(
    ...
    def forward_orig(
        ...
        if self.controlnet_mode_embedder is not None and len(control_type) > 0:
            control_cond = (
                self.controlnet_mode_embedder(
                    torch.tensor(control_type, device=img.device), out_dtype=img.dtype
                )
                .unsqueeze(0)
                .repeat((txt.shape[0], 1, 1))
            )
        ...
        for i in range(len(self.single_blocks)):
            ...
            controlnet_single = controlnet_single + (
                self.controlnet_single_blocks[i](img[:, txt.shape[1] :, ...]),
            )
    ...
    def forward(
        self, x, timesteps, context, y=None, guidance=None, hint=None, **kwargs
    ):
        patch_size = 2
        if self.latent_input:
            hint = comfy.ldm.common_dit.pad_to_patch_size(
                hint, (patch_size, patch_size)
            )
        elif self.mistoline:
            hint = hint * 2.0 - 1.0
        ...
        hint = rearrange(
            hint, "b c (h ph) (w pw) -> b (h w) (c ph pw)", ph=patch_size, pw=patch_size
        )
