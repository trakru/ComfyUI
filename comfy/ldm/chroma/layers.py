...
class DoubleStreamBlock(nn.Module):
    ...
    def forward(
        ...
        img_modulated = torch.addcmul(
            img_mod1.shift, 1 + img_mod1.scale, self.img_norm1(img)
        )
        ...
        txt_modulated = torch.addcmul(
            txt_mod1.shift, 1 + txt_mod1.scale, self.txt_norm1(txt)
        )
        ...
        img.addcmul_(
            img_mod2.gate,
            self.img_mlp(
                torch.addcmul(img_mod2.shift, 1 + img_mod2.scale, self.img_norm2(img))
            ),
        )
        ...
        txt.addcmul_(
            txt_mod2.gate,
            self.txt_mlp(
                torch.addcmul(txt_mod2.shift, 1 + txt_mod2.scale, self.txt_norm2(txt))
            ),
        )
...
class SingleStreamBlock(nn.Module):
    ...
    def forward(
        self, x: Tensor, pe: Tensor, vec: Tensor, attn_mask=None, transformer_options={}
    ) -> Tensor:
        mod = vec
