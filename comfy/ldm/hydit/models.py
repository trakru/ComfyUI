...
class HunYuanDiTBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm1 = norm_layer(
            hidden_size,
            elementwise_affine=use_ele_affine,
            eps=1e-6,
            dtype=dtype,
            device=device,
        )
        ...
        self.default_modulation = nn.Sequential(
            nn.SiLU(),
            operations.Linear(
                c_emb_size, hidden_size, bias=True, dtype=dtype, device=device
            ),
        )
        ...
        if skip:
            ...
            self.skip_linear = operations.Linear(
                2 * hidden_size, hidden_size, dtype=dtype, device=device
            )
        else:
    ...
    def forward(self, x, c=None, text_states=None, freq_cis_img=None, skip=None):
        if self.gradient_checkpointing and self.training:
            return checkpoint.checkpoint(
                self._forward, x, c, text_states, freq_cis_img, skip
            )
...
class FinalLayer(nn.Module):
    ...
    def __init__(
        ...
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(),
            operations.Linear(
                c_emb_size, 2 * final_hidden_size, bias=True, dtype=dtype, device=device
            ),
        )
...
class HunYuanDiT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.use_style_cond:
            ...
            self.style_embedder = operations.Embedding(
    ...
    def forward(
        ...
        _, _, oh, ow = x.shape
        th, tw = (
            (oh + (self.patch_size // 2)) // self.patch_size,
            (ow + (self.patch_size // 2)) // self.patch_size,
        )
        ...
        if self.size_cond:
            image_meta_size = timestep_embedding(image_meta_size.view(-1), 256).to(
                x.dtype
            )  # [B * 6, 256]
        ...
        if self.use_style_cond:
            if style is None:
                style = torch.zeros(
                    (extra_vec.shape[0],), device=x.device, dtype=torch.int
                )
        ...
        for layer, block in enumerate(self.blocks):
            ...
            if ("double_block", layer) in blocks_replace:
                ...
                def block_wrap(args):
                    out = {}
                    out["img"] = block(
                        args["img"], args["vec"], args["txt"], args["pe"], args["skip"]
                    )
                    return out

                out = blocks_replace[("double_block", layer)](
                    {
                        "img": x,
                        "txt": text_states,
                        "vec": c,
