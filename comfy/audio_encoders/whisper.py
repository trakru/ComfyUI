...
class WhisperFeatureExtractor(nn.Module):
    ...
    def __call__(self, audio):
        ...
        mel_spec = self.mel_spectrogram(
            audio.to(self.mel_spectrogram.spectrogram.window.device)
        )[:, :, :-1].to(audio.device)
...
class MultiHeadAttention(nn.Module):
    def __init__(
        self, d_model: int, n_heads: int, dtype=None, device=None, operations=None
    ):
        super().__init__()
...
class EncoderLayer(nn.Module):
    def __init__(
        ...
        self.self_attn = MultiHeadAttention(
            d_model, n_heads, dtype=dtype, device=device, operations=operations
        )
        self.self_attn_layer_norm = operations.LayerNorm(
...
class AudioEncoder(nn.Module):
    def __init__(
        ...
        self.conv1 = operations.Conv1d(
            n_mels, n_state, kernel_size=3, padding=1, dtype=dtype, device=device
        )
        self.conv2 = operations.Conv1d(
            n_state,
            n_state,
            kernel_size=3,
            stride=2,
        ...
        self.embed_positions = operations.Embedding(
            n_ctx, n_state, dtype=dtype, device=device
        )
