...
def clip_preprocess(
    ...
    if not (image.shape[2] == size and image.shape[3] == size):
        ...
        image = torch.nn.functional.interpolate(
            image, size=scale_size, mode="bicubic", antialias=True
        )
...
class ClipVisionModel:
    def __init__(self, json_config):
        ...
        self.model = model_class(
            config, self.dtype, offload_device, comfy.ops.manual_cast
        )
        self.model.eval()
    ...
    def encode_image(self, image, crop=True):
        ...
        out = self.model(
            ...
            intermediate_output="all" if self.return_all_hidden_states else -2,
        )
...
def load_clipvision_from_sd(sd, prefix="", convert_keys=False):
    ...
    # Dinov2
        ...
        json_config = os.path.join(
            os.path.join(os.path.dirname(os.path.realpath(__file__)), "image_encoders"),
            "dino2_giant.json",
        )
