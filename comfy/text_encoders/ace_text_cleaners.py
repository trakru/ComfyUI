...
def japanese_to_romaji(japanese_text):
    ...
    while i < len(japanese_text):
        ...
        if i < len(japanese_text) - 1 and (
            japanese_text[i] == "っ" or japanese_text[i] == "ッ"
        ):
...
def number_to_text(num, ordinal=False):
    ...
    if isinstance(num, float):
        ...
        decimal_text = " point " + " ".join(
            _digit_to_text(int(digit)) for digit in decimal_str
        )
...
def _int_to_text(num):
    ...
    if num < 1000:
        return (
            ones[num // 100]
            + " hundred"
            + (" " + _int_to_text(num % 100) if num % 100 != 0 else "")
        )

    if num < 1000000:
        return (
            _int_to_text(num // 1000)
            + " thousand"
            + (" " + _int_to_text(num % 1000) if num % 1000 != 0 else "")
        )

    if num < 1000000000:
        return (
            _int_to_text(num // 1000000)
            + " million"
            + (" " + _int_to_text(num % 1000000) if num % 1000000 != 0 else "")
        )

    return (
        _int_to_text(num // 1000000000)
        + " billion"
        + (" " + _int_to_text(num % 1000000000) if num % 1000000000 != 0 else "")
    )
