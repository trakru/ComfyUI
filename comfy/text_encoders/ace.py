...
DEFAULT_VOCAB_FILE = os.path.join(
    os.path.join(os.path.dirname(os.path.realpath(__file__)), "ace_lyrics_tokenizer"),
    "vocab.json",
)
    ...
    def __call__(self, string):
        ...
        for line in lines:
            ...
            try:
                ...
                logging.warning(
                    "tokenize error {} for line {} major_language {}".format(
                        e, line, lang
                    )
                )
...
class UMT5BaseModel(sd1_clip.SDClipModel):
    def __init__(
        ...
        textmodel_json_config = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "umt5_config_base.json"
        )
        super().__init__(
            device=device,
            layer=layer,
...
class LyricsTokenizer(sd1_clip.SDTokenizer):
    def __init__(self, embedding_directory=None, tokenizer_data={}):
        tokenizer = os.path.join(
            os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "ace_lyrics_tokenizer"
            ),
            "vocab.json",
        )
        super().__init__(
            ...
            pad_with_end=False,
            embedding_size=1024,
            embedding_key="lyrics",
            tokenizer_class=VoiceBpeTokenizer,
...
class AceT5Tokenizer:
    def __init__(self, embedding_directory=None, tokenizer_data={}):
        self.voicebpe = LyricsTokenizer(
            embedding_directory=embedding_directory, tokenizer_data=tokenizer_data
        )
        self.umt5base = UMT5BaseTokenizer(
            embedding_directory=embedding_directory, tokenizer_data=tokenizer_data
        )

    def tokenize_with_weights(self, text: str, return_word_ids=False, **kwargs):
        out = {}
        out["lyrics"] = self.voicebpe.tokenize_with_weights(
            kwargs.get("lyrics", ""), return_word_ids, **kwargs
        )
        out["umt5base"] = self.umt5base.tokenize_with_weights(
...
class AceT5Model(torch.nn.Module):
    ...
    def encode_token_weights(self, token_weight_pairs):
        ...
        t5_out, t5_pooled = self.umt5base.encode_token_weights(
            token_weight_pairs_umt5base
        )

        lyrics_embeds = torch.tensor(
            list(map(lambda a: a[0], token_weight_pairs_lyrics[0]))
        ).unsqueeze(0)
