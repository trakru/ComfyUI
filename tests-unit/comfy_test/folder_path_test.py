...
def test_annotated_filepath():
    ...
    assert folder_paths.annotated_filepath("test.txt [input]") == (
        "test.txt",
        folder_paths.get_input_directory(),
    )
    assert folder_paths.annotated_filepath("test.txt [temp]") == (
        "test.txt",
...
def test_get_annotated_filepath():
    ...
    assert folder_paths.get_annotated_filepath("test.txt", default_dir) == os.path.join(
        default_dir, "test.txt"
    )
    assert folder_paths.get_annotated_filepath("test.txt [output]") == os.path.join(
        folder_paths.get_output_directory(), "test.txt"
    )
...
def test_add_model_folder_path_append(clear_folder_paths):
    ...
    assert folder_paths.get_folder_paths("test_folder") == [
        "/default/path",
        "/test/path",
    ]
...
def test_add_model_folder_path_re_add_existing_default(clear_folder_paths):
    ...
    folder_paths.add_model_folder_path(
        "test_folder", "/old_default/path", is_default=True
    )
    assert folder_paths.get_folder_paths("test_folder") == [
        "/old_default/path",
        "/test/path",
...
def test_filter_files_extensions():
    ...
    assert folder_paths.filter_files_extensions(files, [".txt"]) == [
        "file1.txt",
        "file4.txt",
    ]
    assert folder_paths.filter_files_extensions(files, [".jpg", ".png"]) == [
        "file2.jpg",
        "file3.png",
    ]
...
def test_base_path_changes(set_base_dir):
    ...
    assert os.path.join(test_dir, "custom_nodes") in folder_paths.get_folder_paths(
        "custom_nodes"
    )
    ...
    for name in [
        ...
        assert folder_paths.get_folder_paths(name)[0] == os.path.join(
            test_dir, "models", name
        )
