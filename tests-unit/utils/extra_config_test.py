...
def test_load_extra_model_paths_expands_userpath(
    ...
    monkeypatch.setattr(
        folder_paths, "add_model_folder_path", mock_add_model_folder_path
    )
    ...
    for actual_call, expected_call in zip(
        mock_add_model_folder_path.call_args_list, expected_calls
    ):
        assert actual_call.args[0] == expected_call[0]
        assert os.path.normpath(actual_call.args[1]) == os.path.normpath(
            expected_call[1]
        )  # Normalize and check the path to check on multiple OS.
...
def test_load_extra_model_paths_expands_appdata(
    ...
    expected_calls = [
        (
            "checkpoints",
            os.path.normpath(os.path.join(expected_base_path, "models/checkpoints")),
            False,
        ),
    ]
...
def test_load_extra_path_config_relative_base_path(
    ...
    def fake_abspath(path):
        ...
        return os.path.join(
            str(tmp_path), path
        )  # Otherwise, do a normal join relative to tmp_path
    ...
    expected_checkpoints = os.path.abspath(
        os.path.join(str(tmp_path), "my_rel_base", "checkpoints")
    )
    expected_some_value = os.path.abspath(
        os.path.join(str(tmp_path), "my_rel_base", "some_value")
    )
...
def test_load_extra_path_config_absolute_base_path(
    ...
    def fake_dirname(path):
        return (
            str(tmp_path) if path.endswith(dummy_yaml_name) else os.path.dirname(path)
        )
