...
class EmptyCosmosLatentVideo(io.ComfyNode):
    ...
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            ...
            inputs=[
                io.Int.Input(
                    "width", default=1280, min=16, max=nodes.MAX_RESOLUTION, step=16
                ),
                io.Int.Input(
                    "height", default=704, min=16, max=nodes.MAX_RESOLUTION, step=16
                ),
                io.Int.Input(
                    "length", default=121, min=1, max=nodes.MAX_RESOLUTION, step=8
                ),
    ...
    def execute(cls, width, height, length, batch_size=1) -> io.NodeOutput:
        latent = torch.zeros(
            [batch_size, 16, ((length - 1) // 8) + 1, height // 8, width // 8],
...
class CosmosImageToVideoLatent(io.ComfyNode):
    ...
    @classmethod
    def execute(
        cls, vae, width, height, length, batch_size, start_image=None, end_image=None
    ) -> io.NodeOutput:
        latent = torch.zeros(
            [1, 16, ((length - 1) // 8) + 1, height // 8, width // 8],
            device=comfy.model_management.intermediate_device(),
        )
        ...
        if start_image is not None:
            latent_temp = vae_encode_with_padding(
                vae, start_image, width, height, length, padding=1
            )
        ...
        if end_image is not None:
            latent_temp = vae_encode_with_padding(
                vae, end_image, width, height, length, padding=0
            )
            latent[:, :, -latent_temp.shape[-3] :] = latent_temp
            mask[:, :, -latent_temp.shape[-3] :] *= 0.0
    ...
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            ...
            inputs=[
                ...
                io.Int.Input(
                    "length", default=93, min=1, max=nodes.MAX_RESOLUTION, step=4
                ),
                io.Int.Input("batch_size", default=1, min=1, max=4096),
                io.Image.Input("start_image", optional=True),
                io.Image.Input("end_image", optional=True),
