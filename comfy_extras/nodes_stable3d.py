...
class StableZero123_Conditioning(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Vae.Input("vae"),
                io.Int.Input(
                    "width", default=256, min=16, max=nodes.MAX_RESOLUTION, step=8
                ),
                io.Int.Input(
                    "height", default=256, min=16, max=nodes.MAX_RESOLUTION, step=8
                ),
                ...
                io.Float.Input(
                    "azimuth", default=0.0, min=-180.0, max=180.0, step=0.1, round=False
                ),
            ],
            outputs=[
    ...
    @classmethod
    def execute(
        cls, clip_vision, init_image, vae, width, height, batch_size, elevation, azimuth
    ) -> io.NodeOutput:
...
class StableZero123_Conditioning_Batched(io.ComfyNode):
    ...
    def execute(
        ...
        cond = torch.cat(
            [comfy.utils.repeat_to_batch_size(pooled, batch_size), cam_embeds], dim=-1
        )
...
class SV3D_Conditioning(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Vae.Input("vae"),
                io.Int.Input(
                    "width", default=576, min=16, max=nodes.MAX_RESOLUTION, step=8
                ),
                io.Int.Input(
                    "height", default=576, min=16, max=nodes.MAX_RESOLUTION, step=8
                ),
                io.Int.Input("video_frames", default=21, min=1, max=4096),
                io.Float.Input(
                    "elevation", default=0.0, min=-90.0, max=90.0, step=0.1, round=False
                ),
            ],
            outputs=[
                io.Conditioning.Output(display_name="positive"),
    ...
    @classmethod
    def execute(
        cls, clip_vision, init_image, vae, width, height, video_frames, elevation
    ) -> io.NodeOutput:
        output = clip_vision.encode_image(init_image)
        pooled = output.image_embeds.unsqueeze(0)
        pixels = comfy.utils.common_upscale(
